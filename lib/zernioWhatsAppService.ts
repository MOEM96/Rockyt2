...
export class ZernioWhatsAppService {
  public static setCachedAccountId(accountId: string, userId?: string, profileId?: string) {
  ...
  public static async listWhatsAppAccounts(profileId?: string, forceRefresh = false): Promise<WhatsAppAccount[]> {
    ...
    // Return stale cache if circuit breaker is open
    if (cacheService.isCircuitOpen('zernio')) {
      const stale = await cacheService.get<WhatsAppAccount[]>(cacheKey);
      if (stale) return stale;
    }

    return await cacheService.fetchWithCoalescing<WhatsAppAccount[]>(
      cacheKey,
      async () => {
        try {
  ...
  public static async getAccountHealth(accountId: string, force = false): Promise<{
    ...
    // Return stale cache if circuit breaker is open
    if (cacheService.isCircuitOpen('zernio')) {
      const stale = await cacheService.get<any>(healthCacheKey);
      if (stale) return stale;
    }
   ...
   */
  public static async createWhatsAppTemplate(accountId: string, payload: {
    name: string;
    ...
    if (payload.message_send_ttl_seconds !== undefined && payload.message_send_ttl_seconds !== null) {
   ...
   */
  public static async getWhatsAppTemplates(accountId: string, filters?: {
    status?: string;
    name?: string;
    language?: string;
  ...
  public static async getWhatsAppTemplate(accountId: string, templateNameOrId: string, language?: string): Promise<any> {
    ...
    const endpoint = isId 
      ? `https://zernio.com/api/v1/whatsapp/templates/id/${templateNameOrId}`
      : `https://zernio.com/api/v1/whatsapp/templates/${templateNameOrId}`;
   ...
   */
  public static async updateWhatsAppTemplate(accountId: string, templateNameOrId: string, updates: {
    components?: any[];
    message_send_ttl_seconds?: number;
    language?: string;
  }): Promise<any> {
    ...
    if (updates.message_send_ttl_seconds !== undefined) {
   ...
   */
  public static async listWhatsAppFlows(accountId: string, forceRefresh = false): Promise<WhatsAppFlow[]> {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) return [];
    ...
    if (cacheService.isCircuitOpen('zernio')) {
      const stale = await cacheService.get<WhatsAppFlow[]>(cacheKey);
      if (stale) return stale;
    }

    return await cacheService.fetchWithCoalescing<WhatsAppFlow[]>(
      cacheKey,
      async () => {
        try {
   ...
   */
  public static async getWhatsAppFlow(flowId: string, accountId: string, forceRefresh = false): Promise<any> {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    ...
    if (cacheService.isCircuitOpen('zernio')) {
      const stale = await cacheService.get<any>(cacheKey);
      if (stale) return stale;
    }

    return await cacheService.fetchWithCoalescing<any>(
      cacheKey,
      async () => {
        try {
