...
var WhatsAppStore = class {
  ...
  getOrCreateConversation(contact, accountId = "acc_primary", profileId = "prof_default", initialReferral) {
  ...
  clearAllConversations(profileId) {
    if (profileId) {
  ...
  appendMessage(msg) {
    ...
    const msgTime = new Date(msg.timestamp || /* @__PURE__ */ new Date()).getTime();
    const existingIdx = list.findIndex((m) => {
      ...
      if (m.direction === msg.direction) {
        ...
        if (msgText && existingText === msgText && Math.abs(msgTime - existingTime) < 12e4) {
          return true;
        }
      }
      return false;
  ...
  cleanDuplicates(conversationId) {
    ...
    for (const m of list) {
      ...
      const dupIdx = cleaned.findIndex((c) => {
        ...
        if (c.direction === m.direction) {
          ...
          if (mText && cText === mText && Math.abs(mTime - cTime) < 12e4) {
            return true;
          }
        }
        return false;
...
// lib/whatsappMessageUtils.ts
function interpolateTemplateText(rawText, params) {
...
function normalizeWhatsAppMessage(m, conversation, fallbackDirection, registeredTemplates) {
  if (!m) {
    return {
      id: `msg_${Date.now()}`,
  ...
  const isFromContact = contactPhone && m.senderId === contactPhone || contactPhone && m.senderPhone === contactPhone || m.source === "contact" || m.direction === "incoming" || m.fromMe === false;
  ...
  let interactiveData = m.interactive_data || void 0;
  ...
  const selectedButtonTitle = m.interactive?.button_reply?.title || m.interactive?.list_reply?.title || m.button?.text || m.metadata?.buttonTitle || m.metadata?.selectedButtonTitle;
  const selectedButtonId = m.interactive?.button_reply?.id || m.interactive?.list_reply?.id || m.button?.payload || m.metadata?.interactiveId || m.metadata?.selectedButtonId;
  ...
  return {
    ...
    timestamp: m.createdAt || m.timestamp || m.sentAt || (/* @__PURE__ */ new Date()).toISOString(),
    sender_name: m.sender_name || m.senderName || (direction === "incoming" ? conversation?.contact?.name || "Customer" : "Support Agent"),
    sender_phone: m.sender_phone || m.senderPhone || (direction === "incoming" ? conversation?.contact?.phone_number : void 0),
    metadata: m.metadata
  };
}

// lib/backendSupabase.ts
...
var ZernioWhatsAppService = class _ZernioWhatsAppService {
  ...
  static async getDefaultAccountId(profileId, userId) {
    ...
    try {
      ...
      if (supabase && userId) {
        if (!profileId) {
          let profQuery = userId.includes("@") ? supabase.from("profiles").select("zernio_profile_id").eq("email", userId.toLowerCase()).maybeSingle() : supabase.from("profiles").select("zernio_profile_id").eq("id", userId).maybeSingle();
  ...
  static async getOrCreateProfileId(userId, userEmail, forceRefresh = false) {
    ...
    try {
      ...
      if (userId && !userId.includes("@")) {
        const { data: pById, error: errById } = await supabase.from("profiles").select("id, email, zernio_profile_id").eq("id", userId).maybeSingle();
      ...
      if (!resolvedProfileId) {
        try {
          ...
          if (verifyRes.ok) {
            ...
            const vMatch = vList.find(
              (p) => cleanEmail && p.name && p.name.trim().toLowerCase() === cleanEmail || p.name && p.name.trim().toLowerCase() === profileDisplayName.toLowerCase()
            );
          ...
          for (const item of list) {
            ...
            if (id && !seenIds.has(id)) {
              ...
              if (accId && accId !== "acc_primary") {
                ...
                _ZernioWhatsAppService.setCachedAccountId(accId);
              }
  ...
  static async sendInboxMessage(params) {
    ...
    if (apiKey && apiKey !== "dummy_dev_key" && params.conversationId) {
      try {
        let effectiveAccountId = params.accountId && params.accountId !== "acc_primary" ? params.accountId : void 0;
        if (!effectiveAccountId) {
          effectiveAccountId = await this.getDefaultAccountId();
        }
        const bodyPayload = {
   ...
   */
  static async createWhatsAppFlow(params) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) {
   ...
   */
  static async uploadWhatsAppFlowJson(flowId, accountId, flowJson) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) {
      return { success: false, error: "Zernio API key not configured" };
    }
   ...
   */
  static async publishWhatsAppFlow(flowId, accountId) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) {
      return { success: false, error: "Zernio API key not configured" };
    }
   ...
   */
  static async sendWhatsAppFlowMessage(params) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) {
      return { success: false, error: "Zernio API key not configured" };
    }
   ...
   */
  static async listWhatsAppFlowResponses(accountId, flowId) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) {
    ...
    try {
      ...
      const responses = rawList.map((r) => ({
        ...
        data: r.data || {},
        received_at: r.receivedAt || (/* @__PURE__ */ new Date()).toISOString()
      }));
      return { success: true, responses };
    } catch (e) {
   ...
   */
  static async getWhatsAppFlowPreview(flowId, accountId, invalidate = false) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) {
   ...
   */
  static async listWhatsAppFlowVersions(flowId, accountId) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) return [];
    try {
      const res = await fetch(
   ...
   */
  static async deprecateWhatsAppFlow(flowId, accountId) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
   ...
   * Delete a draft flow
   */
  static async deleteWhatsAppFlow(flowId, accountId) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) return false;
    try {
      const url = accountId ? `https://zernio.com/api/v1/whatsapp/flows/${encodeURIComponent(flowId)}?accountId=${encodeURIComponent(accountId)}` : `https://zernio.com/api/v1/whatsapp/flows/${encodeURIComponent(flowId)}`;
   ...
   */
  static async registerFlowEncryptionKey(accountId, businessPublicKey) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
   ...
   */
  static async getFlowEncryptionKey(accountId) {
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
    if (!apiKey) return null;
    try {
      const res = await fetch(
...
var ZernioBusinessAgentService = class {
   ...
   */
  static async onboardAgent(accountId, profileId) {
    const apiKey = this.getApiKey();
   ...
   */
  static async updateBusinessInfo(accountId, info, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
   ...
   * Manage FAQs
   */
  static async addFaq(accountId, faq, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
    const newFaq = {
      id: `faq_${Date.now()}`,
  ...
  static async deleteFaq(accountId, faqId, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
   ...
   * Manage Websites
   */
  static async addWebsite(accountId, url, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
    const newWeb = {
      id: `web_${Date.now()}`,
      url,
  ...
  static async deleteWebsite(accountId, websiteId, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
   ...
   * Manage Files
   */
  static async addFile(accountId, file, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
    const newFile = {
      id: `file_${Date.now()}`,
      name: file.name,
  ...
  static async deleteFile(accountId, fileId, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
   ...
   * Manage Skills & Voice
   */
  static async updateSkills(accountId, skills, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
   ...
   */
  static async updateConnector(accountId, connectorId, patch, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
   ...
   */
  static async sendTestMessage(accountId, message, history = [], profileId) {
    const apiKey = this.getApiKey();
    if (apiKey && accountId && accountId !== "acc_primary") {
      try {
   ...
   */
  static async updateSettings(accountId, patch, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
    current.settings = {
      ...current.settings,
      ...patch,
   ...
   * Manage Allowlist
   */
  static async addAllowlistEntry(accountId, phone, name, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
  ...
  static async deleteAllowlistEntry(accountId, entryId, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
   ...
   * Update Budget
   */
  static async updateBudget(accountId, budget, profileId) {
    const current = await this.getFullAgentState(accountId, profileId);
   ...
   */
  static async threadControl(accountId, action, to, metadata) {
    const apiKey = this.getApiKey();
    if (apiKey && accountId && accountId !== "acc_primary") {
      try {
    ...
    if (interactive.type === "nfm_reply" || eventType === "flow.response" || event.nfm_reply) {
      try {
        ...
        if (typeof nfm.response_json === "string") {
          try {
            parsedData = JSON.parse(nfm.response_json);
          } catch {
          }
          ...
          for (const item of liveConvs) {
            ...
            const conv = {
              ...
              last_message: item.lastMessage ? normalizeWhatsAppMessage({
                ...
                sender_name: name,
                sender_phone: phone
              }, void 0, "incoming", whatsappStore.getTemplates()) : void 0
            };
            whatsappStore.saveConversation(conv);
          }
        }
...
var handleSyncChatsAndContacts = async (req, res) => {
  try {
    const { userId, profileId } = await resolveUserProfileId(req);
    const apiKey = process.env.ZERNIO_API_KEY || process.env.ROCKYT_API_KEY;
  ...
  try {
    ...
    const responseData = await cacheService.fetchWithCoalescing(
      ...
      async () => {
        ...
        let directMetaAccount = null;
        if (supabase && targetUserId) {
          try {
            const { data: dbWa } = await supabase.from("whatsapp_accounts").select("waba_id, access_token").eq("user_id", targetUserId).not("access_token", "is", null).maybeSingle();
        ...
        const mergeLiveMetaTemplates = async (liveTemplatesList, sourceAccountId) => {
...
whatsappRouter.post("/api/whatsapp/templates", async (req, res) => {
  try {
    ...
    const defaultAccountId = req.body.accountId || await ZernioWhatsAppService.getDefaultAccountId(profileId, userId);
...
function cleanAndNormalizeMetaFlowJson(rawJson) {
  ...
  cloned.screens.forEach((screen) => {
    (screen.layout?.children || []).forEach((comp) => {
      ...
      if (action && action.name === "navigate" && action.next?.name) {
        const target = cloned.screens.find((s) => s.id === action.next.name);
        if (target) {
  ...
  cloned.screens.forEach((screen) => {
    ...
    for (const [propName, propDef] of Object.entries(screen.data)) {
      if (propDef && typeof propDef === "object" && !propDef.__example__) {
        propDef.__example__ = propName.includes("email") ? "user@example.com" : propName.includes("name") ? "John Doe" : `sample_${propName}`;
      }
    }
  });
...
function validateMetaFlowJson(flowJson) {
  ...
  const screensMap = /* @__PURE__ */ new Map();
  let hasTerminal = false;
  flowJson.screens.forEach((screen, screenIdx) => {
  ...
  flowJson.screens.forEach((screen, screenIdx) => {
    ...
    let hasFooter = false;
    screen.layout.children.forEach((comp, compIdx) => {
...
whatsappRouter.post("/api/whatsapp/flows", async (req, res) => {
  try {
    ...
    if (zernioRes.success && zernioRes.flow?.id) {
      ...
      const randomSuffix = Math.floor(1e11 + Math.random() * 9e11);
    ...
    if (publish) {
      if (validation.valid) {
        ...
        if (supabase) {
          await supabase.from("whatsapp_flows").update({ status: "PUBLISHED", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", createdFlowId);
        }
      }
    }
...
whatsappRouter.get("/api/whatsapp/flows/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const supabase = getBackendSupabaseClient();
    if (!supabase) {
    ...
    const { data: flow, error } = await supabase.from("whatsapp_flows").select("*").eq("id", id).maybeSingle();
    if (error || !flow) {
...
whatsappRouter.post("/api/whatsapp/flows/:id/publish", async (req, res) => {
  try {
    ...
    if (supabase) {
      ...
      const { data: updated, error: updErr } = await supabase.from("whatsapp_flows").update({
        status: "PUBLISHED",
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
...
whatsappRouter.get("/api/whatsapp/flows/:id/preview", async (req, res) => {
  try {
    const { id } = req.params;
...
whatsappRouter.post("/api/whatsapp/flows/send", async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      messageId,
      flow_token: `${flow_id}:${crypto6.randomUUID()}`
    });
  } catch (err) {
...
whatsappRouter.get("/api/whatsapp/flows/:id/responses", async (req, res) => {
  try {
    const { id } = req.params;
    const { profileId } = await resolveUserProfileId(req);
    const defaultAccountId = await ZernioWhatsAppService.getDefaultAccountId(profileId) || "acc_primary";
    ...
    let responses = [];
    if (supabase) {
      const { data, error } = await supabase.from("whatsapp_flow_responses").select("*").eq("flow_id", id).order("received_at", { ascending: false });
      if (!error && Array.isArray(data)) {
        responses = data;
      }
    }
...
whatsappRouter.post("/api/whatsapp/flows/:id/responses", async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const newResponse = {
      ...
      flow_token: `${id}:${crypto6.randomUUID()}`,
      data: formData || {},
      received_at: (/* @__PURE__ */ new Date()).toISOString(),
      created_at: (/* @__PURE__ */ new Date()).toISOString()
    };
...
whatsappRouter.get("/api/whatsapp/flows/:id/versions", async (req, res) => {
  try {
    const { id } = req.params;
    const { profileId } = await resolveUserProfileId(req);
    const defaultAccountId = await ZernioWhatsAppService.getDefaultAccountId(profileId) || "acc_primary";
...
whatsappRouter.post("/api/whatsapp/flows/encryption-key", async (req, res) => {
  try {
    const { profileId } = await resolveUserProfileId(req);
    const defaultAccountId = await ZernioWhatsAppService.getDefaultAccountId(profileId) || "acc_primary";
...
whatsappRouter.get("/api/whatsapp/flows/encryption-key", async (req, res) => {
  try {
    const { profileId } = await resolveUserProfileId(req);
    const defaultAccountId = await ZernioWhatsAppService.getDefaultAccountId(profileId) || "acc_primary";
...
whatsappRouter.get("/api/whatsapp/connect/headless/numbers", async (req, res) => {
  try {
      ...
      if (supabase) {
        ...
        if (userRow) {
          ...
          if (cleanPlatform === "whatsapp" || String(connected).toLowerCase() === "whatsapp") {
            try {
              let accId = accountId ? String(accountId) : void 0;
              let phoneNum = username || "";
              let phoneNumId = accountId ? String(accountId) : void 0;
              let wabaId = void 0;
              if (profileId) {
                try {
